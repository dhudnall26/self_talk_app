...
class _MyHomePageState extends State<MyHomePage> {
                        ...
                        child: ListView(
                          ...
                          children: <Widget>[
                            ...
                            new ListTile(
                              ...
                              title: new Text(
                                topTenKeys.length > 0 ? topTenKeys[0] : '',
                                style: TextStyle(fontSize: 15, color: Colors.blue),
                              ),
                            ),
                            new ListTile(
                              ...
                              leading: new MyBullet(),
                              title: new Text(
                                topTenKeys.length > 1 ? topTenKeys[1] : '',
                                style: TextStyle(fontSize: 15, color: Colors.blue),
                              ),
                            ),
                            new ListTile(
                              ...
                              leading: new MyBullet(),
                              title: new Text(
                                topTenKeys.length > 2 ? topTenKeys[2] : '',
                                style: TextStyle(fontSize: 15, color: Colors.blue),
                              ),
                            ),
                          ],
  ...
  Future<void> storeTranscript(String transcript, String sentiment, String emotion, String keyWords, String category, String date, String email) async {
    final response = await supabase.from('transcript_data').insert([
    {'transcript': transcript, 'sentiment': sentiment, 'emotion': emotion, 'keyWords': keyWords, 'category': category, 'date': date, 'user': email},
    ]);
  }

  Future<void> storeMetricsData(String email, Map<String, num> wordFrequencies, double sad, double happy, double angry, double neutral, var positiveRatio, var negativeRatio, Map<String, int> categoryMap) async {
    final response = await supabase.from('between_sessions').insert([
    {'user': email, 'wordFrequencies': wordFrequencies, 'sad': sad, 'happy': happy, 'angry': angry, 'neutral': neutral, 'positiveRatio': positiveRatio, 'negativeRatio': negativeRatio, 'categoryMap': categoryMap, 'user': email},
    ]);
  }
  ...
  Future<String?> stopRecording() async {
    ...
    List<String> keyWordsList = display_transcript.split(" ");
